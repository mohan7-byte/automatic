...
public class AutomationService extends Service implements SensorEventListener {
    ...
    private boolean hotspotKnown=false, dataKnown=false;
    ...
    private void doAction(String a,String reply){ Prefs p=new Prefs(this); boolean ok=false; if("hotspot".equals(a)){ok=SystemControl.setHotspot(this,!hotspotKnown);if(ok)hotspotKnown=!hotspotKnown;} else if("hotspot_on".equals(a)){ok=SystemControl.setHotspot(this,true);if(ok)hotspotKnown=true;} else if("hotspot_off".equals(a)){ok=SystemControl.setHotspot(this,false);if(ok)hotspotKnown=false;} else if("data".equals(a)){ok=SystemControl.setMobileData(!dataKnown);if(ok)dataKnown=!dataKnown;} else if("data_on".equals(a)){ok=SystemControl.setMobileData(true);if(ok)dataKnown=true;} else if("data_off".equals(a)){ok=SystemControl.setMobileData(false);if(ok)dataKnown=false;} else if("dad".equals(a)){SystemControl.call(this,p.dad());return;} else if("callme".equals(a)){SystemControl.call(this,p.myNumber());return;} else if("status".equals(a)){int b=SystemControl.battery(this);sms(reply,"Redmi 8A: Battery "+b+"% | Hotspot: "+(hotspotKnown?"ON":"unknown")+" | Data: "+(dataKnown?"ON":"unknown")+" | Schedule: "+p.night()+" - "+p.morning());return;}
