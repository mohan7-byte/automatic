...
public class SmsReceiver extends BroadcastReceiver {
    @Override public void onReceive(Context c, Intent i) {
        ...
        Object[] pdus=(Object[])b.get("pdus"); String format=b.getString("format");
        if(pdus==null)return;
        for(Object x:pdus){ SmsMessage m;
            if(android.os.Build.VERSION.SDK_INT>=23) m=SmsMessage.createFromPdu((byte[])x,format); else m=SmsMessage.createFromPdu((byte[])x);
            ...
            s.putExtra("reply", sender); c.startService(s);
        }
    }
    private boolean sameSender(String a,String b){ if(b==null||b.isEmpty())return false; String n1=a.replaceAll("\\D",""); String n2=b.replaceAll("\\D",""); return n1.equals(n2)||(n1.length()>10&&n2.endsWith(n1.substring(n1.length()-10)))||(n2.length()>10&&n1.endsWith(n2.substring(n2.length()-10))); }
}
